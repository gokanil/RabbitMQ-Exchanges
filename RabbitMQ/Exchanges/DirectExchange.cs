        ...
        public static void Consumer(string Key)
        {
            try
            {
                ...
                using (var connection = factory.CreateConnection())
                {
                    ...
                    {
                        channel.ExchangeDeclare(exchange: "direct-exchange-test",
                                                durable: false,
                                                autoDelete: true,
                        ...
                        Console.WriteLine($"'Consumer' başlatıldı. \"direct-exchange-test\" isimli Direct Exchange'e \"{queueName}\" isimli kuyruk key: '{Key}' ile birlikte bind edildi.");
                        Console.ReadLine();
                    }
                }
            }
            ...
            try
            {
                ...
                using (var connection = factory.CreateConnection())
                {
                    using (var channel = connection.CreateModel())
                    {
                        ...
                        channel.BasicPublish(exchange: "direct-exchange-test",
                                             routingKey: Key,
                                             basicProperties: properties,
                                             body: body);

                        Console.WriteLine($"Mesaj key: '{Key}' ile birlikte \"direct-exchange-test\" isimli Direct Exchange'e gönderilmiştir");
                    }
                }
            }
            catch (Exception exception)
